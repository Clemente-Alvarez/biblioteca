...
public interface PrestamoRepository extends JpaRepository<Prestamo, Integer> {
    ...
    @Query(value = "SELECT * FROM prestamos " +
                   "WHERE usuario_id = ?1 AND devuelto = ?2", nativeQuery = true)
    Page<Prestamo> findByUsuarioIdAndDevuelto(Integer usuarioId, Boolean devuelto, Pageable pageable);

    @Query(value = "SELECT * FROM prestamos " +
                   "WHERE usuario_id = ?1 AND devuelto = false ORDER BY fecha_prestamo DESC", nativeQuery = true)
    List<Prestamo> findByUsuarioId(Integer usuarioId);

    @Query(value = "SELECT * FROM prestamos " +
                   "WHERE usuario_id = ?1 AND devuelto = true ORDER BY fecha_devolucion DESC LIMIT 5", nativeQuery = true)
    List<Prestamo> findByUsuarioIdCount(Integer usuarioId);

    @Query(value = "SELECT * FROM prestamos " +
                   "WHERE usuario_id = ?1 AND devuelto = ?2 AND fecha_prestamo = ?3", nativeQuery = true)
    Page<Prestamo> findByUsuarioIdAndDevueltoAndFecha(Integer usuarioId, Boolean devuelto, LocalDate fecha, Pageable pageable);

    @Query(value = "SELECT * FROM prestamos " +
                   "WHERE usuario_id = ?1 AND libro_id = ?2 AND fecha_prestamo = ?3", nativeQuery = true)
