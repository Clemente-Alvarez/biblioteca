...
public interface LibroRepository extends JpaRepository<Libro, Integer> {
    ...
    @Query(value = "SELECT * FROM libros WHERE disponibles > 0", nativeQuery = true)
    Page<Libro> findByDisponibles(Pageable pageable);

    @Query(value = "SELECT * FROM libros WHERE disponibles > 0 AND titulo LIKE '%' || ?1 || '%'", nativeQuery = true)
